# -*- coding: utf-8 -*-
"""
Created on Thu Mar 6 10:45:00 2025
...
class InteractiveTEM(ForwardTEM):
    def __init__(self):
        super().__init__()
        self.wd = Path.cwd()
        ...
        self.project_name = 'TEM_project' # TODO: change default
        self.author = Path.home().name # TODO: change default
    ...
    def show_settings(self):
            if self.c_show_settings.value:
                self.settings = VBox([self.header_settings, self.c_show_settings, self.b_current_key, self.s_loopsize, self.s_current_inj, self.s_timekey, self.b_filter_powerline])
    ...
    def renew_ui(self):
        ...
        self.save_import = VBox([HBox([self.b_save_fig, self.b_save_model, self.b_save_response, self.b_save_all]),
                                    HBox([self.b_load_model_div, self.b_load_model_soda, self.b_load_model_peat, self.b_update])])
        self.ui = VBox([self.settings, self.header, self.slider_display, HBox([self.b_add_layer, self.b_remove_layer]), self.save_import])
    ...
    def save_response(self, click): 
        ...
        save_as_tem(savepath=str(self.responses_path), 
                ...
                setup_device=self.prepare_setup_device(), properties_snd={'rampoff':3e-6, 'current_inj':self.s_current_inj.value},
