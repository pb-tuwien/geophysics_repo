...
class empymod_frwrd(object):
    ...
    def __init__(self, setup_device, setup_solver=None,
        ...
        setup_solver : dictionary, optional
            details, empymod setup_device
        ...
        # self._info_prefix = ' -- INFO: '  # add prefixes to all output/info messages
        ...
        if self.device == 'TEMfast':
            ...
            self.create_props_temfast(ramp_data='salzlacken', show_rampInt=False, times_rx=times_rx)
        else:
    ...
    @staticmethod  # really necessary to use a staticmethod here??
    ...
    def create_props_temfast(self, ramp_data='donauinsel', show_rampInt=False, times_rx=None):
        """
        ...
        path2csv = Path(__file__).parent / f'TEMfast_props/ramp_times_{subfolder}'
    ...
    def calc_response(self, model, unit='res (ohmm)', scale='lin',
        ...
        #  add return values of empymod function to dictionaries for later check, add also do __init__ for better overview
        ...
        EM = empymod.model.bipole(
            ...
            epermH=np.zeros_like(resis), epermV=np.zeros_like(resis),  # fix all!! set electrical permittivity to zero
        ...
        # \_mu for H->B, and i\omega for B->dB/dt.
        EM *= 2j*np.pi*freq*4e-7*np.pi
        self._EM_to_dBdt = np.copy(EM)

        # - check this part in detail to adjust for TEM-fast!
        ...
        delay_rst = prms['delay_rst']      #  check if 0 makes sense, some kind of offset? creates a shift of the data along the time axis
