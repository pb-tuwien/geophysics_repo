...
for idx, thk in enumerate(eval_thk):
    ...
    # plot filtered data second
    first_subzero = np.where(sigIP_mdld < 0)[0][0]
    filt_label = 'filtered data' if idx == 0 else None
    plot_signal(axis=ax,
                time=t_mdld[:first_subzero],
                signal=sigIP_mdld[:first_subzero],
                color=colors[idx],
                marker='.',
                ms=15, lw=3.5,
