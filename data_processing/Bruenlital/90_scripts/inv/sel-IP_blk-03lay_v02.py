...
lambdas = [2000]  # in blocky case: initial lambdas, # TODO smoothness constraint switch!!
cooling_factor = [0.8]
max_iter = 25
mys = [1e3]  # regularization for parameter constraint
noise_floors = np.arange(0.05, 0.25, 0.03)
...
time_ranges = np.array([[20., 300.]])  # in us

thk_lu = (25, 200)    # boundaries for layer thicknesses
res_lu = (100, 50000)  # boundaries for resistivity layers
# boundaries IP params:     m, mpa;    tau, taup;     c
ip_params_lu = np.array([[0.01, 1], [1e-6, 1e-3], [0.01, 1]])

# %% manual initial model and constraints

inlay_thk = np.r_[70.0, 70.0]
inlay_res = np.r_[10000.0, 10000.0, 1000.0]
inlay_mpa = np.r_[0.8, 0.8, 0.01]
inlay_taup = np.r_[5e-4, 5e-4, 1e-6]
inlay_c = np.r_[0.9, 0.9, 0.01]

constr_thk = np.r_[0, 0]
constr_res = np.r_[0, 0, 0]
constr_mpa = np.r_[0, 0, 1]
constr_taup = np.r_[0, 0, 1]
constr_c = np.r_[0, 0, 1]


# %% parameters for IP inversion ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
ip_modeltype = 'mpa'
...
for idx, fname_data_full in enumerate(fnames):
    ...
    #version_filter = f'{version}_tr{int(tr0*1e6)}-{int(np.round(trN*1e6, 0))}us'
            ...
            for log_id, snd in enumerate(snds):  # for running all soundings
                ...
                if ip_modeltype != None:
                    ...
                    fop = temip_block1D_fwd(empy_frwrd, ip_mdltype=ip_modeltype,
                                            nPara=nparams-1, nLayers=nlayers, verbose=True)
                ...
                if show_results:
                    ...
                    for inv_run in range(0, len(snd_log)):
                        ...
                        if ip_modeltype != None: # plot IP case
                            ...
                            if show_initial_model_in_result:
                            ...
                            snd_n.plot_dBzdt(which='observed', ax=ax_all[0],
                                             label='data', color='k', show_sub0_label=False)
                            snd_n.plot_dBzdt(which='calculated', ax=ax_all[0], color='crimson',
                                             marker='x', ls=':', label='inv. response', show_sub0_label=False)
                            ...
                            snd_n.plot_rhoa(which='observed', ax=ax_all[1],
                                             label='data', color='k', show_sub0_label=False)
                            snd_n.plot_rhoa(which='calculated', ax=ax_all[1], color='crimson',
                                            marker='x', ls=':', label='inv. response', show_sub0_label=False)
                            ...
                            snd_n.plot_initial_model(ax=ax_all[2], color='dodgerblue', label='init. model')
                            snd_n.plot_inv_model(ax=ax_all[2], color='magenta', label='inv. model')
                        ...
                        fig_savefid = (savepath_autoplot +
                                        'invrun{:03d}_{:s}_10-model.png'.format(inv_run, snd_name))
                        figfit_savefid = (savepath_autoplot +
                                        'invrun{:03d}_{:s}_00-fit.png'.format(inv_run, snd_name))
                        figjac_savefid = (savepath_autoplot +
                                        'invrun{:03d}_{:s}_90-jac.png'.format(inv_run, snd_name))
...
# max_time = round_up(max(rx_times_sub)*1e6, 100) / 1e6
...
# ttl = (f'inv-run: {inv_run:03d}, sndID: {snd_name}, n_iter: {n_iter}' +
