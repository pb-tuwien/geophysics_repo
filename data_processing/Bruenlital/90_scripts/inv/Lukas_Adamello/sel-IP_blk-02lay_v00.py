...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
sadole testing IP effect from 20 to 40 m depth

less filtering in early times
...
show_initial_model_in_result = True
...
# %% directions data
main = 'C:/Users/jakob/Documents/Meine Ordner/TU/Geophysik/GP_repo_Peter/geophysics_repo/Bruenlital/'
path_data = main + '00_data/selected/'
fnames = ['20240709_sel.tem'] #TODO: change accordingli

# if fnames has length 1
    # you can use multiple time ranges to invert each sounding in the file with a different time range
    # if there is only one time range, it will be used for all soundings in the file
# if fnames has length > 1
    # you can use one time range for each fname


scriptname = os.path.basename('sel-IP_blk-02lay_v00.py') #TODO: get scriptname automatically
...
lambdas = [800]  # in blocky case: initial lambdas, # TODO smoothness constraint switch!!
cooling_factor = [0.8]
max_iter = 25
mys = [1e3]  # regularization for parameter constraint
noise_floors = np.arange(0.08, 0.25, 0.03)
# noise_floors = np.r_[0.08]
...
time_ranges = np.array([[60., 400.], [35., 400.]])  # in us

thk_lu = (0.5, 25)    # boundaries for layer thicknesses
res_lu = (200, 10000)  # boundaries for resistivity layers
# boundaries IP params:     m, mpa;    tau, taup;     c
ip_params_lu = np.array([[0.01, 1], [1e-6, 1e-3], [0.01, 1]])

# %% manual initial model and constraints, 2 layer model, first layer has IP, second no IP
gen_max_depth = 'constraint'
inlay_thk = np.r_[140.0]   # one less than all other params
inlay_res = np.r_[3000.0, 300.0]

inlay_mpa = np.r_[0.7, 0.01]
inlay_taup = np.r_[5e-4, 1e-6]
inlay_c = np.r_[0.9, 0.01]

constr_thk = np.r_[0]
constr_res = np.r_[0, 0]

constr_mpa = np.r_[0, 1]
constr_taup = np.r_[0, 1]
constr_c = np.r_[0, 1]


# %% automatic parameter selection for IP inversion ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# ToDo further testing required, do not use for now
ip_modeltype = 'mpa'
...
for idx, fname_data_full in enumerate(fnames):
    ...
    snd_names = ['ADA1-T8'] #TODO: was bedeutet das?
    snd_positions = []
        ...
        with PdfPages(savepath_data + pdf_filename) as pdf:
            for log_id, snd in enumerate(snds):  # for running all soundings
                ...
                if len(time_ranges) == len(snds):
                    ...
                    print('selected time range (us): ', int(tr0*1e6), '-', int(np.round(trN*1e6, 0)))
                    print('-----------------------------------------------------------------\n')
                    print(f'using an individual time range for each sounding: {fname_data_full}\n')
                elif len(time_ranges) == len(fnames):
                    print('\n-----------------------------------------------------------------')
                    print('selected time range (us): ', int(tr0*1e6), '-', int(np.round(trN*1e6, 0)))
                    print('-----------------------------------------------------------------\n')
                ...
                if show_forward_comp:
                    ...
                    # rawdata first
                    _ = plot_signal(ax_raw[0], times_all, snd.sgnl_o,
                                    marker='o', ls='-', color='grey', sub0color='orange',
                                    label='data raw')

                    ax_raw[0].loglog(times_all, snd.error_o,  # noise
                                      'd', ms=4, ls=':',
                                      color='grey', alpha=0.5,
                                      label='noise raw')

                    _ = plot_rhoa(ax_raw[1], times_all, snd.rhoa_o,
