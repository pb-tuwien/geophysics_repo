...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
treibach mapping inversion with IP only P01
    cs01
    layers 2, 3 and 4 with IP effects
    
    cs10
    layers 2, 3, 4 and 5 with IP effects

    cs11
    layers 2 - 6 with IP effects
...
time_ranges = np.array([[8., 400.]])  # in us

thk_lu = (1, 10)    # boundaries for layer thicknesses
res_lu = (0.5, 500)  # boundaries for resistivity layers


# %% manual initial model and constraints
gen_max_depth = 'constraint'
inlay_thk = np.r_[2.3, 8.0, 8.0, 8.0, 6.0, 6.0, 9.0, 9.0]
inlay_res = np.r_[30.0, 8.0, 8.0, 100.0, 40.0, 25.0, 25.0, 20.0, 20.0]

inlay_mpa = np.r_[0.0, 0.4, 0.4, 0.4, 0.2, 0.2, 0.0, 0.0, 0.0]
inlay_taup = np.r_[1e-6, 5e-4, 5e-4, 5e-4, 5e-4, 5e-4, 1e-6, 1e-6, 1e-6]
inlay_c = np.r_[0.01, 0.5, 0.5, 0.5, 0.5, 0.5, 0.01, 0.01, 0.01]

constr_thk = np.r_[1, 0, 0, 0, 0, 0, 0, 0]
constr_res = np.r_[0, 0, 0, 0, 0, 0, 0, 0, 0]

constr_mpa = np.r_[1, 0, 0, 0, 0, 0, 1, 1, 1]
constr_taup = np.r_[1, 0, 0, 0, 0, 0, 1, 1, 1]
constr_c = np.r_[1, 0, 0, 0, 0, 0, 1, 1, 1]
...
# boundaries for IP params: m, mpa;    tau, taup;     c
