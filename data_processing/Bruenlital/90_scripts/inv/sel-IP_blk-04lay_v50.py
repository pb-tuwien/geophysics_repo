...
noise_floors = np.arange(0.12, 0.29, 0.03)

gen_max_depth = 'constraint'  # gen_max_depth = 'individual'  # or use a float value that limits the actual depth!
gen_nlayers = 3

# time_ranges = np.array([[8., 200.], [8., 400.], [8., 600.]])  # in us
time_ranges = np.array([[60., 400.], [20., 400.]])  # in us

thk_lu = (30, 80)    # boundaries for layer thicknesses
res_lu = (300, 50000)  # boundaries for resistivity layers
# boundaries IP params:     m, mpa;    tau, taup;     c
ip_params_lu = np.array([[0.01, 1], [1e-7, 1e-3], [0.01, 1]])

# %% manual initial model and constraints
gen_max_depth = 'constraint'
inlay_thk = np.r_[50.0, 50.0, 50.0]
inlay_res = np.r_[3000.0, 3000.0, 3000.0, 300.0]
inlay_mpa = np.r_[0.7, 0.9, 0.7, 0.01]
inlay_taup = np.r_[5e-5, 5e-5, 5e-5, 1e-7]
inlay_c = np.r_[0.8, 0.95, 0.9, 0.01]

constr_thk = np.r_[0, 0, 0]
constr_res = np.r_[0, 0, 0, 0]
constr_mpa = np.r_[0, 0, 0, 1]
constr_taup = np.r_[0, 0, 0, 1]
constr_c = np.r_[0, 0, 0, 1]
